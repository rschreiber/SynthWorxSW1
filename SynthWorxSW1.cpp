...
SynthWorxSW1::SynthWorxSW1(void *instance):
  ...
  m_note_on(-1)
{
  ...
  pKnobControl = new IKnobCustomControl(this, 214, 150, kParamCutoffFrequency, &knobBitmap);
  pKnobControl->SetTooltip("Cutoff Frequency");
  pGraphics->AttachControl(pKnobControl);

  pKnobControl = new IKnobCustomControl(this, 214, 280, kParamResonance, &knobBitmap);
  pKnobControl->SetTooltip("Resonance");
  pGraphics->AttachControl(pKnobControl);

  pKnobControl = new IKnobCustomControl(this, 214, 410, kParamLFOFrequency, &knobBitmap);
  pKnobControl->SetTooltip("LFO Rate");
  pGraphics->AttachControl(pKnobControl);

  pKnobControl = new IKnobCustomControl(this, 214, 540, kParamLFOAmplitude, &knobBitmap);
...
void SynthWorxSW1::SetSampleRate(double rate)
{
  IPlug::SetSampleRate(rate);
  m_synth->SetSampleRate(rate);
}

void SynthWorxSW1::SetBlockSize(int size)
{
  IPlug::SetBlockSize(size);
  m_midi_queue.Resize(GetBlockSize(), false);
}

void SynthWorxSW1::OnParamChange(int index)
{
  switch (index)
  {
    case kParamEnvelope:
...
void SynthWorxSW1::Reset()
{
  m_synth->Reset();
}

void SynthWorxSW1::ProcessMidiMsg(const IMidiMsg *msg)
{
  m_midi_queue.Add(msg);
}

void SynthWorxSW1::ProcessMidiQueue(const IMidiMsg *msg)
{
...
void SynthWorxSW1::ProcessDoubleReplacing(const double *const *inputs, double *const *outputs, int samples)
{
...
bool SynthWorxSW1::OnGUIRescale(int wantScale)
{
